...
NumPlots = 500000         # Number of times the figure is updated
dT       = 0.025     # Time step
...
__kernel void SimulationKernel (__global float* M)
{
    ...
    size_t column   = current%Grid_Width;
            ...
            {
             float dMdt = -M[current]*(M[current] - 0.5)*(M[current] - 1.0) + D*d2_dxy2(M);
             
        		 M[current]=M[current]+dMdt*dT;
            }
...
for ii in range(NumPlots):
    ...
    for jj in range(int(EndTime/dT)):
        program.SimulationKernel(queue, global_size, local_size, M_g)
    ...
    # Updating the graphs
    #misc.imsave('20000%08d.png'%(ii),M_host.reshape(Size,Size))
    imageio.imwrite('20000%08d.png'%(ii),M_host.reshape(Size,Size))
