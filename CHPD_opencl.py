...
EndTime  = 1000.0 # Total simulation time
...
for i in range(1,int(Size*Size)):
    if(M[i]<0.99 and M[i]>0.01):
        M[i]=0.333333333333
...
print(" * Cahn-Hilliard PD model    2019                        * ");
print(" * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n");
...
Devices  = platform[1].get_devices()
...
M[current] = M[current] + (dM + 0.0001*M[current]*(1.0 - 3.0*M[current]))*dT ;
